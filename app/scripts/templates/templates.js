...
module.run(["$templateCache", function($templateCache) {
  ...
  $templateCache.put("note.html",
    ...
    "        <button class=\"plain-button reply-button\" ng-click=\"addReply($index, $event, note.id, note.contributor, note.step_id)\">Reply</button>\n" +
    ...
    "        </div>\n" +
    "        <note ng-repeat=\"reply in note.replies\" ng-if=\"note.replies.length > 0 && canShowNote(reply)\" note=\"reply\" delete-it = \"deleteIt\" rate-it = \"rateIt\" add-reply=\"addReply\" can-show-note=\"canShowNote\" user=\"user\" date=\"date\"></note>\n" +
    "    </div>\n" +
    "</div>\n" +
    "");
}]);
